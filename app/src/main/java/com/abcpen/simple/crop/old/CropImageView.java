...
public class CropImageView extends ImageViewTouchBase {
	...
	public void setFixWidth(boolean b) {
		fixWidth = b;
	}
		...
		if (mBitmapDisplayed.getBitmap() != null) {
            ...
            try {
                if (getDrawable() instanceof BitmapDrawable) {
                    ...
                    float s1 = getWidth() / (float)oriBitmap.getWidth();
                    float s2 = getHeight() / (float)oriBitmap.getHeight();
                    float s3 = Math.min(s1, s2);
                    if (s3 > 1f) {
                        setMinScale(s3);
                    }
                }
		...
		case MotionEvent.ACTION_MOVE:
			if (mMotionHighlightView != null) {
				mMotionHighlightView.handleMotion(mMotionEdge, event.getX()
						- mLastX, event.getY() - mLastY, fixWidth);
				mLastX = event.getX();
				mLastY = event.getY();
	...
	public void centerBasedOnHighlightView(HighlightView hv) {
		...
		if ((Math.abs(zoom - getScale()) / zoom) > .01) {
			float[] coordinates = new float[] { hv.mCropRect.centerX(),
					hv.mCropRect.centerY() };
			getImageMatrix().mapPoints(coordinates);
			zoomTo(zoom, coordinates[0], coordinates[1], 300F, hv);
		}
	...
	public Bitmap creatNewPhoto() {
		...
		Bitmap bitmap = mBitmapDisplayed.getBitmap();
