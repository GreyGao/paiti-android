...
abstract class ImageViewTouchBase extends ImageView {
	...
	protected float maxZoom() {
		...
		float fw = (float) mThisWidth * 3 / (float) mBitmapDisplayed.getWidth();
		float fh = (float) mThisHeight * 3 / (float) mBitmapDisplayed.getHeight();
		float max = Math.min(Math.min(fw, fh) , 6);
		return max;
	}
	...
	public abstract void ensureVisible(HighlightView hv);
